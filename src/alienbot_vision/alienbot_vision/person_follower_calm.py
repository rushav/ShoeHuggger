#!/usr/bin/env python3
"""
AlienBot Person Follower — Follow (Calm) Mode
===============================================
Calmly follows a detected person from a distance. Stops driving when close
enough and gently flaps wings while stationary near the person.

Same YOLO ankle detection and obstacle avoidance as Annoy mode, but with
slower speeds, distance-based stop/resume logic, and wings that only flap
when the robot has stopped near the person.
...
MISS_LIMIT        = 20     # frames without detection before dropping lock
...
class PersonFollowerCalm(Node):
    """ROS2 node that calmly follows a person from a distance (Follow mode)."""
    ...
    def process_frame(self, frame: np.ndarray) -> None:
        """Run YOLO inference and publish velocity + wing commands with distance control."""
        if not self.running:
            return
        ...
        if all_feet:
            ...
            if box_area > STOP_AREA:
                ...
                if not self.is_stopped_near:
                    ...
                    self.get_logger().info(
                        f'Close enough (area:{box_area:.3f}) — stopping, flapping gently.')
                ...
                self.get_logger().info(
                    f'Person moved away (area:{box_area:.3f}) — resuming follow.')
            ...
            # Visuals — green when following, cyan when stopped
