#!/usr/bin/env python3
"""
AlienBot Person Follower — Annoy Mode
======================================
Aggressively chases a detected person and flaps wings faster as it gets closer.

Uses YOLOv8n-pose to detect ankle keypoints, locks onto one target's feet,
and drives toward them with reactive LiDAR obstacle avoidance. Wing flap
speed is proportional to the target's apparent size (closer = faster flap).

When no person is detected for several seconds, the robot spins in place
to search for a new target.

Subscribes:
    /image_raw/compressed (sensor_msgs/CompressedImage) — camera feed
    /scan (sensor_msgs/LaserScan) — LiDAR for obstacle avoidance

Publishes:
    /cmd_vel (geometry_msgs/Twist) — wheel velocity commands
    /follower_image (sensor_msgs/Image) — annotated debug image
...
LINEAR_SPEED      = 0.20   # base forward speed (m/s)
ANGULAR_SPEED     = 0.6    # base turning speed (rad/s)
...
ANKLE_CONF_THRESH = 0.4    # minimum keypoint confidence to consider an ankle visible
FEET_PAD          = 30     # pixels of padding around detected feet bounding box
MISS_LIMIT        = 20     # frames without detection before dropping lock (~5s at ~4fps)
...
OBSTACLE_STOP_DIST = 0.35  # meters — stop all forward motion if anything this close
...
# YOLOv8 pose keypoint indices for ankles
LEFT_ANKLE  = 15
RIGHT_ANKLE = 16
# ─────────────────────────────────────────────────────────────────────────────
...
class PersonFollower(Node):
    """ROS2 node that chases a person using YOLO ankle detection (Annoy mode)."""
    ...
    def __init__(self) -> None:
        ...
        self.locked_area: float | None = None
        self.miss_count = 0
        ...
        # Search state
        self.target_lost_time: float | None = None
    ...
    def image_callback(self, msg: CompressedImage) -> None:
        """Decode compressed image and store as latest frame."""
    ...
    # ── Obstacle avoidance ────────────────────────────────────────────────────
    def compute_obstacle_avoidance(self) -> tuple[float, float, str]:
        """
    ...
    def infer_loop(self) -> None:
        """Continuously grab latest frame and run YOLO inference."""
        while self.running and rclpy.ok():
            with self.frame_lock:
    ...
    # ── Feet extraction ───────────────────────────────────────────────────────
    def extract_feet(self, result, w: int, h: int) -> list[tuple]:
        """Extract ankle keypoints from YOLO pose result as (cx, cy, x1, y1, x2, y2)."""
        feet_list = []
    ...
    def compute_flap_speed(self, box_area: float) -> float:
    ...
    def process_frame(self, frame: np.ndarray) -> None:
        """Run YOLO inference on a frame and publish velocity + wing commands."""
        if not self.running:
            return
        ...
        if all_feet:
            ...
            # Velocity component for predictive turning
