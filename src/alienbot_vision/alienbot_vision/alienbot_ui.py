...
AlienBot Control UI
===================
Tkinter GUI that launches alongside RViz. Manages robot mode switching
via subprocess control.

Buttons / Keyboard Shortcuts:
    1  TELEOP     — launches teleop_hold in xterm for manual arrow-key control
    2  ANNOY      — launches person_follower (chase + aggressive wing flap)
    3  FOLLOW     — launches person_follower_calm (calm follow from distance)
    4  STOP ALL   — kills all modes, zeros motors and wings (also Space/Esc)
...
class AlienBotUI:
    """Tkinter control panel for switching AlienBot operating modes."""
    ...
    def __init__(self) -> None:
        ...
        self.btn_teleop = tk.Button(
            ...
            cursor='hand2', command=self.start_teleop, height=2
        )
        self.btn_teleop.pack(fill=tk.X, pady=5)
        tk.Label(btn_frame, text='Manual control with arrow keys',
                 font=info_font, fg='#555555', bg='#1a1a2e').pack(pady=(0, 10))

        self.btn_annoy = tk.Button(
            btn_frame, text='2  ANNOY MODE', font=btn_font,
            fg='white', bg='#e94560', activebackground='#c73a52',
            activeforeground='white', relief=tk.FLAT,
            cursor='hand2', command=self.start_annoy, height=2
        )
        self.btn_annoy.pack(fill=tk.X, pady=5)
        tk.Label(btn_frame, text='Chase person + flap wings aggressively',
                 font=info_font, fg='#555555', bg='#1a1a2e').pack(pady=(0, 10))
        ...
        self.btn_follow = tk.Button(
            ...
            activeforeground='white', relief=tk.FLAT,
            cursor='hand2', command=self.start_follow, height=2
        )
        self.btn_follow.pack(fill=tk.X, pady=5)
        tk.Label(btn_frame, text='Calm follow from distance + gentle flap',
                 font=info_font, fg='#555555', bg='#1a1a2e').pack(pady=(0, 10))
        ...
        self.btn_stop = tk.Button(
            ...
            activeforeground='white', relief=tk.FLAT,
            cursor='hand2', command=self.stop_all, height=2
        )
        self.btn_stop.pack(fill=tk.X, pady=5)
        tk.Label(btn_frame, text='Emergency stop — kills all modes + motors',
                 font=info_font, fg='#555555', bg='#1a1a2e').pack(pady=(0, 10))
    ...
    def _launch_mode(self, name: str, cmd: str) -> None:
