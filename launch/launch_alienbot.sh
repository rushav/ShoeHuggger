...
if ros2 topic list 2>/dev/null | grep -q "/cmd_vel"; then
    echo "  OK Robot topics detected"
else
    echo "  WARNING No robot topics found — is the Pi running?"
fi

# Start robot_state_publisher + SLAM
echo "[2/4] Starting robot_state_publisher + SLAM..."
ros2 launch "$SCRIPT_DIR/alienbot_slam.launch.py" &
SLAM_PID=$!
sleep 3

# Start RViz
echo "[3/4] Starting RViz..."
rviz2 -d "$SCRIPT_DIR/alienbot.rviz" &
RVIZ_PID=$!
sleep 2
